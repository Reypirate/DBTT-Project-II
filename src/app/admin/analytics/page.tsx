...
export default function DeepAnalyticsPage() {
  ...
  const topSurgingProducts = useMemo(() => mlAdapters.adaptedSurgingProducts, [mlAdapters]);
                    ...
                    <div className="space-y-6">
                      <Alert className="bg-primary/5 border-primary/20">
                        ...
                        <AlertDescription className="text-xs mt-1.5 text-text-main/80 leading-relaxed">
                          ...
                          <strong>{mlAdapters.positiveShare.toFixed(1)}% positive sentiment</strong>{" "}
                          across deterministic customer responses.
                        </AlertDescription>
                      </Alert>
              ...
              <CardContent className="p-0 sm:p-6">
                ...
                <div className="mt-8 p-5 rounded-xl bg-secondary/5 border border-secondary/20">
                  <p className="text-sm text-text-main/80 leading-relaxed italic">
                    <strong>Insight:</strong> Classification modeling indicates Overseas users have
                    a {mlAdapters.churnGapPct >= 0 ? "lower" : "higher"} average churn probability
                    by {Math.abs(mlAdapters.churnGapPct).toFixed(1)}% versus Local users (
                    {(mlAdapters.overseasChurnProbability * 100).toFixed(1)}% vs{" "}
                    {(mlAdapters.localChurnProbability * 100).toFixed(1)}%), supporting
                    region-targeted retention campaigns.
                  </p>
                </div>
              </CardContent>
            </Card>
