...
export function RemembranceCard({ item }: { item: any }) {
  ...
  useEffect(() => {
    const fetchTip = async () => {
      try {
        const prompt = `Provide a 1-sentence respectful ancestral advice/tip for honoring ${item.name} (${item.description || "family tradition"}).`;
  ...
  return (
    <Card className="border border-neutral-main shadow-sm overflow-hidden group hover:border-primary/40 transition-all duration-300">
      <CardHeader className="bg-surface pb-4">
        ...
        <CardDescription className="flex items-center gap-2 text-text-main/60 text-sm mt-1">
          <Calendar className="size-4" />
          {item.date}
          <span className="text-secondary/80">•</span>
          <span className="text-xs font-semibold text-secondary">{item.lunarDate}</span>
        </CardDescription>
      </CardHeader>
